...
require_once("durjoysoft_sms.class.php");
...
class Functions
{
    ...
    public function checkHooks($hooks = null)
    {
        ...
        foreach ($hooks as $hook) {
            $sql = "SELECT `id` FROM `durjoysoft_sms_templates` WHERE `name` = '" . $hook['function'] . "' AND `user_type` = '" . $hook['type'] . "' LIMIT 1";
            ...
            if ($num_rows == 0) {
                if ($hook['type']) {
                    ...
                    insert_query("durjoysoft_sms_templates", $values);
                    $i++;
                }
            } else {
                $values = array(
                ...
                update_query("durjoysoft_sms_templates", $values, "name = '" . $hook['name'] . "'");
            }
        }
    ...
    public function getTemplateDetails($template = null)
    {
        ...
        $result = select_query("durjoysoft_sms_templates", "*", $where);
    ...
    {
        $result = select_query("durjoysoft_sms_settings", "*", array('id' => 1));
    ...
    public function send()
    {
        ...
        $send = new durjoysoft_sms($settings['api_key']);

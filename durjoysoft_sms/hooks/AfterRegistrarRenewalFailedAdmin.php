...
if (!function_exists('AfterRegistrarRenewalFailedAdmin')) {
    function AfterRegistrarRenewalFailedAdmin($args)
    {
        ...
        if (empty($settings['api_key'])) {
            logActivity('durjoysoft_sms - AfterRegistrarRenewalFailedAdmin :  ' . 'No API Key Provided', 0);
            return null;
        }
        ...
        foreach ($admin_numbers as $gsm) {
            if (!empty($gsm)) {
                ...
                if (!$class->validatePhoneNumber($gsm)) {
                    logActivity('durjoysoft_sms - AfterRegistrarRenewalFailedAdmin :  ' . 'Invalid phone number Provided', 0);
                    continue;
                }

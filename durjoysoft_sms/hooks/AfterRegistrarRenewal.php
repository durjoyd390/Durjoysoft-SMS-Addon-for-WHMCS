...
if (!function_exists('AfterRegistrarRenewal')) {
    function AfterRegistrarRenewal($args)
    {
        ...
        if (empty($settings['api_key'])) {
            logActivity('durjoysoft_sms - AfterRegistrarRenewal :  ' . 'No API Key Provided', 0);
            return null;
        }
        ...
        if ($num_rows == 1) {
            ...
            if (!$class->validatePhoneNumber($UserInformation['gsmnumber'])) {
                logActivity('durjoysoft_sms - AfterRegistrarRenewal :  ' . 'Invalid phone number Provided', 0);
                return null;
            }

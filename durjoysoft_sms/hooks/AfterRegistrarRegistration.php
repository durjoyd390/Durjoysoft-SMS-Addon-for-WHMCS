...
if (!function_exists('AfterRegistrarRegistration')) {
    function AfterRegistrarRegistration($args)
    {
        ...
        if (empty($settings['api_key'])) {
            logActivity('durjoysoft_sms - AfterRegistrarRegistration :  ' . 'No API Key Provided', 0);
            return null;
        }
        ...
        if ($num_rows == 1) {
            ...
            if (!$class->validatePhoneNumber($UserInformation['gsmnumber'])) {
                logActivity('durjoysoft_sms - AfterRegistrarRegistration :  ' . 'Invalid phone number Provided', 0);
                return null;
            }

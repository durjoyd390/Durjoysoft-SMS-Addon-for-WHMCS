    ...
    function AfterModuleChangePackage($args)
    {
        ...
        if ($type == "hostingaccount") {
            ...
            if (empty($settings['api_key'])) {
                logActivity('durjoysoft_sms - AfterModuleChangePackage :  ' . 'No API Key Provided', 0);
                return null;
            }
        } else {
            return null;
        ...
        if ($num_rows == 1) {
            ...
            if (!$class->validatePhoneNumber($UserInformation['gsmnumber'])) {
                logActivity('durjoysoft_sms - AfterModuleChangePackage :  ' . 'Invalid phone number Provided', 0);
                return null;
            }

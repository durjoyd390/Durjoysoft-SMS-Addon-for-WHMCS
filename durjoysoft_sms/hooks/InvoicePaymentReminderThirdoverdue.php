    ...
    function InvoicePaymentReminderThirdoverdue($args)
    {
        if ($args['type'] == "thirdoverdue") {
            ...
            if (empty($settings['api_key'])) {
                logActivity('durjoysoft_sms - InvoicePaymentReminderThirdoverdue :  ' . 'No API Key Provided', 0);
                return null;
            }
        } else {
            return false;
        ...
        if ($num_rows == 1) {
            ...
            if (!$class->validatePhoneNumber($UserInformation['gsmnumber'])) {
                logActivity('durjoysoft_sms - InvoicePaymentReminderThirdoverdue :  ' . 'Invalid phone number Provided', 0);
                return null;
            }

...
if (!function_exists('AcceptOrderSMS')) {
    function AcceptOrderSMS($args)
    {
        ...
        if (empty($settings['api_key'])) {
            logActivity('durjoysoft_sms - AcceptOrderSMS :  ' . 'No API Key Provided', 0);
            return null;
        }
        ...
        if ($num_rows == 1) {
            ...
            if ($UserInformation['country'] != 'BD') {
                logActivity('durjoysoft_sms - AcceptOrderSMS :  ' . 'SMS not sent, This client is not from Bangladesh', 0);
                return null;
            }

            if (!$class->validatePhoneNumber($UserInformation['gsmnumber'])) {
                logActivity('durjoysoft_sms - AcceptOrderSMS :  ' . 'Invalid phone number Provided', 0);
                return null;
            }

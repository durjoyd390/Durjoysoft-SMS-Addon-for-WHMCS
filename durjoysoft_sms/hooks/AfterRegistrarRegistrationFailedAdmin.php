...
if (!function_exists('AfterRegistrarRegistrationFailedAdmin')) {
    function AfterRegistrarRegistrationFailedAdmin($args)
    {
        ...
        if (empty($settings['api_key'])) {
            logActivity('durjoysoft_sms - AfterRegistrarRegistrationFailedAdmin :  ' . 'No API Key Provided', 0);
            return null;
        }
        ...
        foreach ($admin_numbers as $gsm) {
            if (!empty($gsm)) {
                ...
                if (!$class->validatePhoneNumber($gsm)) {
                    logActivity('durjoysoft_sms - AfterRegistrarRegistrationFailedAdmin :  ' . 'Invalid phone number Provided', 0);
                    continue;
                }
